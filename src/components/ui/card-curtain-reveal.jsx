...
const CardCurtain = React.forwardRef(({ className, inverted, ...props }, ref) => {
  const { isMouseIn } = useCardCurtainRevealContext()
  ...
  return (
    <motion.div
      ...
      variants={inverted ? curtainOpenVariants : curtainVariants}
      animate={inverted ? (isMouseIn ? "hidden" : "visible") : (isMouseIn ? "visible" : "hidden")}
      {...props}
    />
  )
})
